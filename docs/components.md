...
Auto-generated from `examples/components/*.html`.
...
Conditional display based on URL params, hash, media query, CSS support, language, or network. Attributes: has-param, has-hash, has-media, has-support, has-lang, has-network, match-any.

**Usage**

```html
<show-when has-param="debug">Shown only when ?debug exists</show-when>
```

**Implementation notes**

MutationObserver on attributes checks each condition. Toggles hidden attribute. Listens to hashchange, media change, online/offline events.
...
Three-way theme switcher: light, dark, auto (system). Saves to localStorage, follows OS in auto mode via light-dark().

**Usage**

```html
<theme-switch> (round fab that cycles) · <theme-switch full> (three inline buttons)
```

**Implementation notes**

Without `full`: a single round fab button that cycles light → auto → dark. With `full`: the three options shown inline. Both set <meta name="color-scheme"> and store in localStorage; auto mode clears the saved key and light-dark() follows the OS.
